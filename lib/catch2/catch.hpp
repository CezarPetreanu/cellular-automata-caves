...
struct is_unique<T0, T1, Rest...> : std::integral_constant<bool, !std::is_same<T0, T1>::value && is_unique<T0, Rest...>::value && is_unique<T1, Rest...>::value>
{};
	...
	#define INTERNAL_CATCH_NTTP_1(signature, ...)                                                                                                                                                                   \
		...
		constexpr auto rewrap(List<__VA_ARGS__>, Elements... elems) noexcept->decltype(append(TypeList<Container<__VA_ARGS__>> {}, rewrap<Container>(elems...)))                                                    \
		{                                                                                                                                                                                                           \
			return {};                                                                                                                                                                                              \
		}                                                                                                                                                                                                           \
...
std::vector<double> resolution(int k)
{
	...
	deltas.reserve(k);
	std::transform(std::next(times.begin()), times.end(), times.begin(), std::back_inserter(deltas), [](TimePoint<Clock> a, TimePoint<Clock> b) { return static_cast<double>((a - b).count()); });
...
struct Parser : ParserBase
{
	...
	void writeToStream(std::ostream& os) const
	{
		...
		{
			auto row =
				TextFlow::Column(cols.left).width(optWidth).indent(2) + TextFlow::Spacer(4) + TextFlow::Column(cols.right).width(consoleWidth - 7 - optWidth);
			os << row << std::endl;
		}
	}
...
Catch::Totals runTests(std::shared_ptr<Config> const& config)
{
	...
	{
		bool matching = (!testSpec.hasFilters() && !testCase.isHidden()) || (testSpec.hasFilters() && matchTest(testCase, testSpec, *config));
...
bool SectionTracker::isComplete() const
{
	...
	if ((m_filters.empty() || m_filters[0] == "") || std::find(m_filters.begin(), m_filters.end(), m_nameAndLocation.name) != m_filters.end())
